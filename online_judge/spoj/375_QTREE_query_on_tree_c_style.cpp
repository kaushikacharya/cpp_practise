...
//#include <istream>
#include <cassert>
// Note: reading commands has issues
...
void process(FILE* pFile)
{
	...
	//test:
	graph.update_edge_cost(0,5);
	graph.traversal(0,4);
	printf("%ld\n",graph.max_cost_in_path(0,4));
	int gh = 0;
...
int main(int argc, char* argv[])
{
	...
	if (READ_FROM_FILE)
	{
		pFile = fopen("D:/cpp_practise/online_judge/spoj/375_QTREE_query_on_tree_input.txt","r");
